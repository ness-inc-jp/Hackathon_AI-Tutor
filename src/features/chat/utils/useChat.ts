...
const diaryModeSystemPrompt = `
#命令書:
あなたは、アメリカ人のプロの英語講師です。
以下の制約条件をもとに、diaryについて会話して下さい。
...
const diaryModePromptTempate = ChatPromptTemplate.fromPromptMessages([
  SystemMessagePromptTemplate.fromTemplate(diaryModeSystemPrompt),
  new MessagesPlaceholder("history"),
  HumanMessagePromptTemplate.fromTemplate("{input}"),
]);
...
export const useChat = () => {
  ...
  useEffect(() => {
    ...
    let firstHumanPrompt = "";
    let firstAiMessage = "";
    if (diary) {
      chain.prompt = diaryModePromptTempate;
      firstHumanPrompt = `Have a conversation about my diary.\n#Diary\n\n${diary}`;
      firstAiMessage = "Hi!\nI have read your wonderful diary.";
    } else {
      chain.prompt = freeModePromptTempate;
      firstHumanPrompt = "Hello!";
    }

    setTimeout(() => {
      if (firstAiMessage) {
        const firstAiChatMessage: AIChatMessage = {
          role: "ai",
          content: firstAiMessage,
          audioUrl: "",
        };
        setMessages([...messages, firstAiChatMessage]);
      }
  ...
  const conversation = async (inputMessage: string, isFirst: boolean) => {
    ...
    const res = await chain.call({ input: inputMessage }, [
      {
